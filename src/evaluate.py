...
import os
import torch.nn.functional as F
from utils import pad_instr_tokens  
...
class Planner:
    ...
    def process_instruction(self, instruction):
        ...
        padded_tokens, _ = pad_instr_tokens(tokens, maxlength=80)  
    ...
    def run_episode(self, episode_idx: int) -> None:
        ...
        while self.step_counter < self.max_steps and not done:
            rgb_t = self.rgb_transform(rgb_image_np).unsqueeze(0).to(self.env.unwrapped.device)  
            depth_t = self.process_depth(depth_map)  
        ...
        progress = (initial_distance - final_distance + 1) / initial_distance if not success else 1 
